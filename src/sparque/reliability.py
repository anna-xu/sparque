...
def calc_reliability(subj_name, df, subj_column_name, func_conn_col_start=3):
    ...
    We define reliability as the correlation coefficient between the (Fisher
    z-transformed) edge lists of the connectivity matrices. If there are more than 2
    runs, the output will be the list of unique edge values of a correlation
    coefficient matrix, with each value of the matrix representing the reliability
    between two runs.
    ...
    df : dataframe object
        Dataframe of edge lists of connectivity matrices across runs with subject and run
        columns specified (currently expected output from `func_conn.conn_from_dir`)
    ...
    if corr_conn.shape == ():
        raise ValueError(
            'Could not compute reliabilities across sessions. This could be due to at '
            'least one of your subjects having only one session or one of your '
            'connectivity values is not valid.'
        )

    return corr_conn[np.triu_indices_from(corr_conn, 1)]


def reliability_multiple_subjects(
    df, subj_column_name, output_filename, func_conn_col_start=3
):
    """
    Calculate reliability of parcellated connectome across runs for multiple subjects.
    Sessions with nan values in edge lists are dropped and recorded in
    ``reliability_log_{current date and time}.txt``
    ...
    ----------
    df : dataframe object
        Dataframe of subjects, sessions, and edge lists of connectivity matrices across
        runs (currently expected output from ``func_conn.conn_from_dir``)
    subj_column_name : str
    ...
    for subject in set(list(subjects)):
        reliabilities = calc_reliability(
            subject, df, subj_column_name, func_conn_col_start
        )
...
def get_reliability(avg_corr_connmats_df):
    ...
    for i, subject in enumerate(avg_corr_connmats_df['subject']):
        ...
        reliability_df['mean_reliability'].append(
            np.mean(avg_corr_connmats_df['reliabilities'].iloc[i])
        )
