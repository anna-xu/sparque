...
def run_all_metrics(
    ...
    for _, curr_metric in enumerate(metrics):
        ...
        if curr_metric == 'fc_homogeneity':
            temp_eval_data_df = fc_homogeneity.average_fc_homogeneity(
                scans, parc_name, parc_fdata, f'{parc_name}_fch_{datetime.now()}.csv'
            )
            ...
            temp_avg_corr_connmat_df = reliability.reliability_multiple_subjects(
                reliability_conn_file,
                'subject',
                f'{parc_name}_reliabilities_{datetime.now()}.h5',
                func_conn_col_start,
            )
            ...
            _, DCBC_average_df = dcbc.run_DCBC(
                dist_file,
                parc_name,
                loaded_surface_parc,
            ...
            eval_data['L_DCBC'] = [
                temp_eval_data_df['DCBC'][temp_eval_data_df['hemisphere'] == 'L']
            ]
            eval_data['R_DCBC'] = [
                temp_eval_data_df['DCBC'][temp_eval_data_df['hemisphere'] == 'R']
            ]
...
def run_parcel_eval(
    ...
    metrics : array_like
        List of metrics as str to run (currently only accepts 'fc_homogeneity', 'dcbc',
        'reliability', 'svc)
    scans (optional): array_like
        List of scans to analyze
    parcellation_df : dataframe object
        Dataframe containing parcellation name, associated parcellation file, number of
        parcels, associated surface image file (left), associated surface image file
        (right). You can run `parcellation_dict` and look at `parcellation_df` for
        example of default.
    dist_file (optional): str
        Location of distance matrix file for DCBC. Please see DCBC GitHub repo for more
        information on obtaining distance matrix file
        (https://github.com/DiedrichsenLab/DCBC). Since this file is big, it cannot be
        readily uploaded onto GitHub repo.
    func_conn_file (optional): str or Dataframe
        csv file containing subject, session, and upper triangle of functional
        connectivity matrix; to be used to measure reliability and classification
        accuracy
    func_conn_col_start : int
        Index of where the edge list values start. If output from
        ``func_conn.conn_from_dir``, edge list values start at column 3.
    ...
    for _, curr_parc in enumerate(parcellations):
        ...
        if parcellation_df is not None:
            parcel_surface_L_data = parcellation_df['surface_file_L'][
                parcellation_df['parcellation'] == curr_parc
            ].iloc[0]

            parcel_surface_R_data = parcellation_df['surface_file_R'][
                parcellation_df['parcellation'] == curr_parc
            ].iloc[0]

            func_conn_file = parcellation_df['func_conn_file'][
                parcellation_df['parcellation'] == curr_parc
            ].iloc[0]

        if 'fc_homogeneity' in metrics:
            parcellation_file = parcellation_df['parc_file'][
                parcellation_df['parcellation'] == curr_parc
            ].iloc[0]
        ...
        if 'dcbc' in metrics:
            parc_L_gii, parc_R_gii = nb.load(parcel_surface_L_data), nb.load(
                parcel_surface_R_data
            )
            surface_parc = [parc_L_gii, parc_R_gii]
        else:
            surface_parc = None

        temp_eval_data = run_all_metrics(
            scans,
            metrics,
            curr_parc,
            parc_fdata,
