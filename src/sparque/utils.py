...
def convert_mni_to_fslr32k(scan_filename, save=False, filename=None):
    """
...
def get_mask(mask, data):
    ...
    if mask == 'MNI' and importlib.util.find_spec('templateflow'):
        ...
        img_mask = tflow.get(
            'MNI152NLin2009cAsym', desc='brain', suffix='mask', resolution=2
        )
    else:
        img_mask = mask
...
def filter_ts_by_std(data, data_ts_to_filter_from, std_tol_max):
    """
    Filters out voxels with time series containing standard deviation of less than
    ``std_tol_max``
    """
